...
import com.byandev.consumersubmission.database.DatabaseContract
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        holder.itemView.apply {
            if (cursor?.moveToPosition(position)!!) {
                tvUserName.text = DatabaseContract.UserColumns.getColumnString(cursor!!, DatabaseContract.UserColumns.LOGIN)
                val builder = Picasso.Builder(context)
                builder.build()
                    .load(DatabaseContract.UserColumns.getColumnString(cursor!!, DatabaseContract.UserColumns.AVATAR_URL))
                    .fit()
                    .into(imgUsers)
            }
        }
